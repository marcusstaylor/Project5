...
public class DataProcessor {
     ...
     * constructor for DataProcessor, and it will create the appropriate fields
     * 
     * @param inSongs
    ...
    /**
     * reorderByData(int) will return to the client code
     * a LinkedList<Song> that is sorted
     ...
     *            2 for the proportion of students who fell into the category
     *            and liked the song
     * @return the proportion of student
     */
...
//////////////////////////////////////////////////////////////////////////////
    ...
    /**
     * getCompareValue(Song, int) returns the position of the song in the
     * LinkedList of the category it's being compared in
     * 
     * @param song
     *            the song being evaluated
     * @param category
     *            1 search by artist name
     *            2 search by song title
     *            3 search by genre
    ...
    /**
     * orderArtist() will order their alphabetical ranking. IE an artist will be
     * rated lower is their name comes before others in an alphabetical sorting
     * of the names.
     * 
    ...
    /**
     * orderGenre() will return a linkedlist containing all of the song genres
     * of which the list of songs belong
     * 
     * @return a string containing a sorted list of genres
     */
    private LinkedList<String> orderGenre() {
        ...
        int smallestIndex;
        for (Iterator<Song> iter = this.songList.iterator(); iter.hasNext();) {
            String buffer2 = iter.next().getGenre();
            if (!genres.contains(buffer2)) {
                genres.add(buffer2);
            }
        }
        for (int i = 0; i < genres.getSize(); i++) {
            smallestIndex = i;
            String buffer = genres.get(smallestIndex);
     ...
     * number of students in the student list
     * 
     * @param song
     *            the song being evaluated
     * @param val
     ...
     *            3 -> Region (NE US, SE US, rest of US, outside of US)
     * @param resp
     ...
     */
    protected double getTotalOfResponses(
        Song song,
        int enumer,
        int val,
        int resp) {
        ...
        for (Iterator<Student> iter = possible.iterator(); iter.hasNext();) {
            buf = iter.next();
            if (buf.getResponses()[songIndex].equals("Yes") || buf
                .getResponses()[songIndex].equals("No")) {
                total = total + 1.0;
            }
        }
        return total;
